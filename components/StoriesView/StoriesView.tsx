...
const StoriesView = ({ story, onClose, onNext, onPrev }: Props) => {
  ...
  const storiesData = story.stories.map((_story) => ({
    ...
    header: {
      ...
      subheading: `Posted ${formatTime(new Date(_story.posted_on))}`,
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-90 transition-opacity duration-500 ${
        !story
